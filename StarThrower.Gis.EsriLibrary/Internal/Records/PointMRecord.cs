...
{
    /// <summary>
    /// Represents a PointM shape record (a point with a measure value). Unimplemented:
    /// <c>ParseBytes</c> throws <see cref="NotImplementedException"/>, and
    /// <c>GetBytes</c>/<c>GetLengthInBytes</c> only serialize the 4-byte shape-type code.
    /// Tracked in issue #19.
    /// </summary>
    internal sealed class PointMRecord : StarThrower.Gis.EsriLibrary.Internal.GeographyFileRecordContent
    {
        #region Construction

        internal PointMRecord(byte[] bytes)
        {
            this.ShapeType = StarThrower.Gis.EsriLibrary.ShapeType.PointM;
            ParseBytes(bytes);
        }
        ...
        internal override StarThrower.Gis.GeoUtilities.GeoRectangle Extent
        {
            get
            {
                //TODO: #19 — implement Extent once ParseBytes populates X/Y data
        ...
        {
            StarThrower.Gis.GeoUtilities.Shapes.PointMShape result = new StarThrower.Gis.GeoUtilities.Shapes.PointMShape();
            //TODO: #19 — populate result with parsed X, Y, M values
            return result;
        }
