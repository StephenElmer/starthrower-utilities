...
{
    /// <summary>
    /// Represents a MultiPointZ shape record (a multipoint with elevation and measure
    /// values). Unimplemented: <c>ParseBytes</c> throws <see cref="NotImplementedException"/>,
    /// and <c>GetBytes</c>/<c>GetLengthInBytes</c> only serialize the 4-byte shape-type code.
    /// Tracked in issue #20.
    /// </summary>
    internal sealed class MultiPointZRecord : StarThrower.Gis.EsriLibrary.Internal.GeographyFileRecordContent
    {
        #region Construction

        internal MultiPointZRecord(byte[] bytes)
        {
            this.ShapeType = StarThrower.Gis.EsriLibrary.ShapeType.MultiPointZ;
            ParseBytes(bytes);
        }
        ...
        internal override StarThrower.Gis.GeoUtilities.GeoRectangle Extent
        {
            get
            {
                //TODO: #20 — implement Extent once ParseBytes populates bounding box data
        ...
        {
            StarThrower.Gis.GeoUtilities.Shapes.MultipointZShape result = new StarThrower.Gis.GeoUtilities.Shapes.MultipointZShape();
            //TODO: #20 — populate result with parsed point, Z, and M data
            return result;
        }
