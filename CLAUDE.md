...
| `StarThrower.DateTimeUtilities` | `DTUtil.Iso8601ToDateTime` | `DateTimeOffset.Parse(..., DateTimeStyles.AssumeUniversal \| DateTimeStyles.AdjustToUniversal).UtcDateTime` | **Done.** Original hand-rolled parser stripped the TZD offset off but never applied it — a real bug masked by the existing tests only exercising `+00:00`. Body replaced with a call to the BCL parser; marked `[Obsolete]`. Tests fleshed out with positive/negative offset and `Z`-designator cases; existing tests left unmodified. |
